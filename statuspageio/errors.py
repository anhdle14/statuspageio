class ConfigError(Exception):
    """
    Exception raised in case of invalid client configuration
    e.g. no access token provided, invalid access token, invalid base url etc.
    """
...
class BaseError(Exception):
    ...
    * :class:`ServerError <ServerError>`
    ...
    :attribute list errors: List of :class:`Munch <munch.Munch>`
    objects repsenting returned errors.
...
class RequestError(BaseError):
    """
    Exception raised if the request was invalid
    e.g. unknown query parameterm, invalid request's body envelope etc.
    """
